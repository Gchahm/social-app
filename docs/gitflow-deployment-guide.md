...
2. [GitFlow Branch Strategy](#gitflow-branch-strategy)
...
export class BeStack extends Stack {
  constructor(scope: Construct, id: string, props: BeStackProps) {
    ...
    // Outputs with environment suffix
...
// Get environment from context (passed via CLI)
...
// Environment-specific AWS account/region configuration
const envConfig = {
  dev: {
  ...
  prod: {
    // Recommended: Use separate AWS account for production
    account: process.env.PROD_AWS_ACCOUNT || process.env.CDK_DEFAULT_ACCOUNT,
...
new BeStack(app, 'BeStack', {
  ...
  description: `Backend stack for ${environment} environment`,
  tags: {
    Environment: environment,
...
### Option 1: Single AWS Account (Simpler)

Use the same AWS account with environment-specific resource naming.

### Option 2: Multiple AWS Accounts (Recommended for Production)

Separate AWS accounts for better isolation:
- Dev Account: Development and Staging
